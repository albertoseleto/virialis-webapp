    ...
    if uploaded_file is not None:
        ...
        if gas_type == 'A2B2':
            ...
            for T in range(50, 1000, step):
                ...
                xmin = np.array([0, 0, 0, 0], np.float64)
                xmax = np.array([10, np.pi, np.pi, 2*pi], np.float64)
                st.write('before')
                val, err = cubature(integrand_sphere, 4, 1, xmin, xmax)
                st.write('after')
                print('Approximated: {0}'.format(val))

                B_cubature.append(val)
            ...
            fig3.add_trace(
                go.Scatter(x=Tstr, y=B_cubature,
                           name='B cubature')
            )
            '''fig3.add_trace(
