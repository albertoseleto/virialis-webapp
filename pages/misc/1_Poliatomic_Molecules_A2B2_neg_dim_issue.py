...
def ILJ():
    ...
    class drv:
        ...
        def d1_H(r):
            d1_H = num_diff.derivative(
                LC1.UH, r, method='central', p=1e-8)
            return d1_H

        def d1_L(r):
            d1_L = num_diff.derivative(
                LC1.UL, r, method='central', p=1e-8)
            return d1_L

        def d1_Ta(r):
            d1_Ta = num_diff.derivative(
                LC1.UTa, r, method='central', p=1e-8)
            return d1_Ta

        def d1_Tb(r):
            d1_Tb = num_diff.derivative(
                LC1.UTb, r, method='central', p=1e-8)
            return d1_Tb

        def d1_X(r):
            d1_X = num_diff.derivative(
                LC1.UX, r, method='central', p=1e-8)
            return d1_X

        def d1_Z(r):
            d1_Z = num_diff.derivative(
                LC1.UZ, r, method='central', p=1e-8)
            return d1_Z
        ...
        def d1th_a(r, th_a, th_b, phi):
            d1th_a = 1/3*np.sin(2*th_a) * (LC1.UH(r) -
                                            LC1.UL(r)+LC1.UTa(r)-2*LC1.UTb(r)+LC1.UX(r))
            -1/504*(-6*(1+3*np.cos(2*th_b)) * np.sin(2*th_a) - 6*(1-np.cos(2*th_b)) * np.cos(2*phi) * np.sin(2*th_a)
                    + 12*np.cos(2*th_a) * np.cos(phi) * np.sin(2*th_b)) * (13*LC1.UH(r)-LC1.UL(r)+7*LC1.UTa(r)+7*LC1.UTb(r)-14*LC1.UX(r)-12*LC1.UZ(r))
            +1/35*(-6 * (1+3*np.cos(2*th_b)) * np.sin(2*th_a) + (1-np.cos(2*th_b)) * np.cos(2*phi) * np.sin(2*th_a)
                    - 16*np.cos(2*th_a) * np.cos(phi) * np.sin(2*th_b)) * (LC1.UH(r)+LC1.UL(r)-2*LC1.UZ(r))
            +1/360*(-6 * (1+3*np.cos(2*th_b)) * np.sin(2*th_a) + 6*(1-np.cos(2*th_b)) * np.cos(2*phi) * np.sin(2*th_a)
                    + 24*np.cos(2*th_a) * np.cos(phi) * np.sin(2*th_b)) * (4*LC1.UH(r)-LC1.UL(r)-5*LC1.UTa(r)-5*LC1.UTb(r)-5*LC1.UX(r)+12*LC1.UZ(r))

            return d1th_a

        def d1th_b(r, th_a, th_b, phi):
            d1th_b = 1/3*np.sin(2*th_b) * (LC1.UH(r) -
                                            LC1.UL(r)-2*LC1.UTa(r)+LC1.UTb(r)+LC1.UX(r))
            -1/504*(12*np.cos(2*th_b) * np.cos(phi) * np.sin(2*th_a) - 6*(1+3*np.cos(2*th_a)) * np.sin(2*th_b) - 6*(1-np.cos(
                2*th_a)) * np.cos(2*phi) * np.sin(2*th_b))*(13*LC1.UH(r)-LC1.UL(r)+7*LC1.UTa(r)+7*LC1.UTb(r)-14*LC1.UX(r)-12*LC1.UZ(r))
            +1/35*(-16*np.cos(2*th_b) * np.cos(phi) * np.sin(2*th_a)
                    - 6 * (1+3*np.cos(2*th_a)) * np.sin(2*th_b) + (1-np.cos(2*th_a)) * np.cos(2*phi) * np.sin(2*th_b))*(LC1.UH(r)+LC1.UL(r)-2*LC1.UZ(r))
            +1/360*(24*np.cos(2*th_b) * np.cos(phi) * np.sin(2*th_a) - 6 * (1+3*np.cos(2*th_a)) * np.sin(2*th_b)
                    + 6*(1-np.cos(2*th_a)) * np.cos(2*phi) * np.sin(2*th_b)) * (4*LC1.UH(r)-LC1.UL(r)-5*LC1.UTa(r)-5*LC1.UTb(r)-5*LC1.UX(r)+12*LC1.UZ(r))
            return d1th_b

        def d1phi(r, th_a, th_b, phi):
            d1phi = -1/504*(-6*np.sin(2*th_a) * np.sin(2*th_b) * np.sin(phi) + 6*(1-np.cos(2*th_a)) * (1-np.cos(
                2*th_b)) * np.sin(2*phi)) * (13*LC1.UH(r)-LC1.UL(r)+7*LC1.UTa(r)+7*LC1.UTb(r)-14*LC1.UX(r)-12*LC1.UZ(r))
            +1/35*(8*np.sin(2*th_a) * np.sin(2*th_b) * np.sin(phi) - (1-np.cos(2*th_a))
                    * (1-np.cos(2*th_b)) * np.sin(2*phi)) * (LC1.UH(r)+LC1.UL(r)-2*LC1.UZ(r))
            + 1/360*(-12*np.sin(2*th_a) * np.sin(2*th_b) * np.sin(phi) - 6*(1-np.cos(2*th_a)) * (1-np.cos(
                2*th_b)) * np.sin(2*phi))*(4*LC1.UH(r)-LC1.UL(r)-5*LC1.UTa(r)-5*LC1.UTb(r)-5*LC1.UX(r)+12*LC1.UZ(r))
            return d1phi
        ...
        def d2th_a(r, th_a, th_b, phi):
            d2th_a = 2/3*np.cos(2*th_a) * (LC1.UH(r) -
                                            LC1.UL(r)+LC1.UTa(r)-2*LC1.UTb(r)+LC1.UX(r))
            + 1/504*(12*np.cos(2*th_a) * (1+3*np.cos(2*th_b)) + 12*np.cos(2*th_a) * (1-np.cos(2*th_b)) * np.cos(2*phi)
                        + 24*np.cos(phi) * np.sin(2*th_a) * np.sin(2*th_b)) * (13*LC1.UH(r)-LC1.UL(r)+7*LC1.UTa(r)+7*LC1.UTb(r)-14*LC1.UX(r)-12*LC1.UZ(r))
            + 1/35*(-12*np.cos(2*th_a) * (1+3*np.cos(2*th_b)) + 2*np.cos(2*th_a) * (1-np.cos(2*th_b)) * np.cos(2*phi)
                    + 32*np.cos(phi) * np.sin(2*th_a) * np.sin(2*th_b)) * (LC1.UH(r)+LC1.UL(r)-2*LC1.UZ(r))
            + 1/360*(-12*np.cos(2*th_a) * (1+3*np.cos(2*th_b)) + 12*np.cos(2*th_a) * (1-np.cos(2*th_b)) * np.cos(2*phi)
                        - 48*np.cos(phi) * np.sin(2*th_a) * np.sin(2*th_b)) * (4*LC1.UH(r)-LC1.UL(r)-5*LC1.UTa(r)-5*LC1.UTb(r)-5*LC1.UX(r)+12*LC1.UZ(r))
            return d2th_a

        def d2th_b(r, th_a, th_b, phi):
            d2th_b = 2/3*np.cos(2*th_b) * (LC1.UH(r) -
                                            LC1.UL(r)-2*LC1.UTa(r)+LC1.UTb(r)+LC1.UX(r))
        ...
        def d2phi(r, th_a, th_b, phi):
            d2phi = 1/504*(-12*(1-np.cos(2*th_a)) * (1-np.cos(2*th_b)) * np.cos(2*phi)
                            + 6*np.cos(phi) * np.sin(2*th_a) * np.sin(2*th_b)) * (13*LC1.UH(r)-LC1.UL(r)+7*LC1.UTa(r)+7*LC1.UTb(r)-14*LC1.UX(r)-12*LC1.UZ(r))
            + 1/35*(-2*(1-np.cos(2*th_a)) * (1-np.cos(2*th_b)) * np.cos(2*phi)
                    + 8*np.cos(phi) * np.sin(2*th_a) * np.sin(2*th_b)) * (LC1.UH(r)+LC1.UL(r)-2*LC1.UZ(r))
            + 1/360*(-12*(1-np.cos(2*th_a)) * (1-np.cos(2*th_b)) * np.cos(2*phi)
                        - 12*np.cos(phi) * np.sin(2*th_a) * np.sin(2*th_b)) * (4*LC1.UH(r)-LC1.UL(r)-5*LC1.UTa(r)-5*LC1.UTb(r)-5*LC1.UX(r)+12*LC1.UZ(r))
            return d2phi
    ...
    def integrand_c1(x,mi):  # primeira correção quantica
                r = x[0]
                th_a = x[1]
                th_b = x[2]
                phi = x[3]
    ...
    def integrand_c2(x,I1,I2):  # segunda correção quantica
        r = x[0]
        th_a = x[1]
        th_b = x[2]
        phi = x[3]
    ...
    def integrand_c3(x,mi1,mi2):  # segunda correção quantica
        r = x[0]
        th_a = x[1]
        th_b = x[2]
        phi = x[3]
    ...
    def integrand_c4(x,mi):  # quarta correção quantica
        r = x[0]
        th_a = x[1]
        th_b = x[2]
        phi = x[3]
