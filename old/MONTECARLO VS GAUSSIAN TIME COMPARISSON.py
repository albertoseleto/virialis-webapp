# -*- coding: utf-8 -*-
"""
Created on Sat Jul 30 17:35:19 2022
...
for T in range(10, 100, 5): #CALCULOS QUE SERÃO FEITOS NO PRIMEIRO INTERVALO DE TEMPERATURA
    ...
    corr1i1 =monte_carlo_uniform(f1correção,a=0,b=2,n=1800) #integração
   
    B1corr1=((N_A*10**-24*(h**2))*(corr1i1))/(48*(irk**3)*(T**3)*mi) #coef do virial da primeira correção



    corr2i1 =monte_carlo_uniform(f2correção,a=0,b=2,n=1800) #integração
   ...
   # d1 = decimal.Decimal(B1)
    ...
    corr3i1  =monte_carlo_uniform(f3correção,a=0,b=2,n=1800) #integração
    #print(integral1)
    #print(integral1, T)
    B1corr3=((-N_A*10**-24)*(corr3i1))/(48*(irk**3)*(T**3))

   
    corr4i1 =monte_carlo_uniform(f4correção,a=0,b=2,n=1800) #integração
 
 
    B1corr4=((-N_A*10**-24*(h**4))*(corr4i1))/(1920*(irk**4)*(T**4)*(mi**2)) #*mi
    
    
    Btotal1= (Bmonte1 + B1corr1 + B1corr2 + B1corr3 + B1corr4) #soma de cada um dos coef encontrados resultando no coeficiente total
    


    
    Tmonte.append(T) #temperaturas usadas são colocadas na lista
    
    Bmonte.append(Btotal1) #Coef total achado é alocado na lista
    print('B=', Btotal1, 'para T=',T)
...
for T in range(100, 300, 25):  #CALCULOS QUE SERÃO FEITOS NO SEGUNDO INTERVALO DE TEMPERATURA
    ...
    corr1i2 = monte_carlo_uniform(f1correção, a=0,b=2,n=1800) #integração
   
    B2corr1=((N_A*10**-24*(h**2))*(corr1i2))/(48*(irk**3)*(T**3)*mi) #coef do virial da primeira correção



    corr2i2 = monte_carlo_uniform(f2correção, a=0,b=2,n=1800)
    ...
    corr3i2 = monte_carlo_uniform(f3correção, a=0,b=2,n=1800)

    B2corr3=((-N_A*10**-24)*(corr3i2))/(48*(irk**3)*(T**3)) #coef do virial da terceira correção

   
    corr4i2 = monte_carlo_uniform(f4correção, a=0,b=2,n=1800)
  
    B2corr4=((-N_A*10**-24*(h**4))*(corr4i2))/(1920*(irk**4)*(T**4)*(mi**2)) #coef do virial da quarta correção
    
    
    Btotal2= (Bmonte2 + B2corr1 + B2corr2 + B2corr3 + B2corr4) #soma de cada um dos coef encontrados resultando no coeficiente total
    ...
    Bmonte.append(Btotal2) #Coef total achado é alocado na lista
    print('B=',Btotal2, 'para T=',T)
...
for T in range(300, 500, 50):  #CALCULOS QUE SERÃO FEITOS NO TERCEIRO INTERVALO DE TEMPERATURA
    ...
    corr1i3 =monte_carlo_uniform(f1correção,a=0,b=2,n=1800)
   
    B3corr1=((N_A*10**-24*(h**2))*(corr1i3))/(48*(irk**3)*(T**3)*mi) #coef do virial da segunda correção



    corr2i3 =monte_carlo_uniform(f2correção,a=0,b=2,n=1800)
    #print(integral1)
    #print(integral1, T)
    B3corr2=((-N_A*10**-24)*(corr2i3))/(48*(irk**3)*(T**3)) #coef do virial da segunda correção
   ...
   # d1 = decimal.Decimal(B1)
    ...
    corr3i3 =monte_carlo_uniform(f3correção,a=0,b=2,n=1800)
    #print(integral1)
    #print(integral1, T)
    B3corr3=((-N_A*10**-24)*(corr3i3))/(48*(irk**3)*(T**3)) #coef do virial da terceira correção

   
    corr4i3 =monte_carlo_uniform(f4correção,a=0,b=2,n=1800)
    #print(integral1)
    #print(integral1, T)
    B3corr4=((-N_A*10**-24*(h**4))*(corr4i3))/(1920*(irk**4)*(T**4)*(mi**2)) #coef do virial da quarta correção
    
    
    Btotal3= (Bmonte3 + B3corr1 + B3corr2 + B3corr3 + B3corr4)  #soma de cada um dos coef encontrados resultando no coeficiente total
    ...
    Bmonte.append(Btotal3) #Coef total achado é alocado na lista
    print('B=',Btotal3, 'para T=',T)
...
print('GAUSSSS'+ str(total_time))
print("MONTE CARLOOOOOOO"+ str(total_time2))
...
ax1.plot(Tmonte, Bmonte, label = 'dados monte carlo') #SEM CORREÇÕES QUANTICAS
...
##2.9900364875793457 GAUSSIAN 
#0.15160083770751953 MONTE CARLO
