    ...
    if uploaded_file is not None:
        ...
        if gas_type == 'A2B2':
            ...
            def integrand_c1(x):  # primeira correção quantica
                ...
                c1 = np.pi/24 * constants.N_A * (constants.h**2)/mi * (
                    constants.rk/T)**3 * np.sin(th_a) * np.sin(th_b) * np.exp(-F) * d_1r**2 * r**2
            ...
            def integrand_c2(x):  # segunda correção quantica
                ...
                VL1 = -d_1th_a * np.cos(th_a)/np.sin(th_a) - \
                    d_2th_a - d_2phi/(np.sin(th_a))**2
                VL2 = -d_1th_b * np.cos(th_b)/np.sin(th_b) - \
                    d_2th_b - d_2phi/(np.sin(th_b))**2

                c2 = -np.pi/24 * constants.N_A * constants.h**2 * r**2 * \
                    np.exp(-F) * (VL1/mi1 + VL2/mi2) * (constants.rk/T)**2
            ...
            def integrand_c3(x):  # terceira correção quantica nossa, segunda do maximiliano
                ...
                c3a_max = np.exp(-F) * r**2 * (d_2r**2/10 + d_1r**2/(5*r**2) +
                                               d_1r**3/(9/constants.rk*r*T) - d_1r**4/(72*(1/constants.rk*T)**2))
                # c3b_max =
                ...
                c3 = -np.pi/48 * constants.N_A * \
                    (constants.h**4)/mi**2 * (c3a_max) * \
                    (constants.rk/T)**4 * np.sin(th_a) * np.sin(th_b)
            ...
            def integrand_c4(x):  # quarta correção quantica
                ...
                c4 = (VL1 + VL2) * np.exp(-F) * (-np.pi/24) * \
                    constants.N_A * constants.h**2/mi * (constants.rk/T)**2
            ...
            for T in range(50, 1000, step):
                ...
                st.write('result VEGAS geral =', -result.mean - result_c1.mean -
